...
| `ROOT_PATH` | Префикс подкаталога за обратным прокси, например `/adrobot`. Нужен только кнопке Try it out в Swagger UI, на маршрутизацию не влияет | пусто |
...
**За обратным прокси, в подкаталоге.** Интерфейс целиком построен на относительных адресах, поэтому работает из любого каталога без настройки: например, `https://host.example/adrobot/`. Важно открывать адрес с завершающим слэшем, иначе браузер будет искать стили и скрипты уровнем выше. Переменная `ROOT_PATH=/adrobot` нужна только Swagger UI и только когда прокси отрезает префикс: с ней в схему попадает запись `servers`, и кнопка Try it out шлёт запросы на верные адреса. В маршрутизацию префикс не передаётся намеренно: Starlette в этом случае считает, что префикс придёт в адресе запроса, и примонтированный каталог начинает искать файлы на уровень глубже — интерфейс открылся бы без стилей и скриптов.
